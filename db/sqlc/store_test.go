...
func TestTransferTx(t *testing.T){
	...
	// design to connect concurrent go routines:
	// allow them to share data without explicit locking
	// one channel for receiving errors and one channel for TransferTxResults:
	...
	for i := 0; i< n; i++{
		...
		fmt.Println(">> in goroutine! loop results:", fromAccount.Balance, toAccount.Balance)
		// check accounts' balance:
		diff1 := account1.Balance - fromAccount.Balance
		diff2 := toAccount.Balance - account2.Balance
		require.Equal(t, diff1, diff2)
		require.True(t, diff1 > 0)
		require.True(t, diff1 % amount == 0) // after first transaction 1*amount, second: 2*amount, third: 3 * amount, ....
